...
function __construct(){
	...
	public function index()
	{
        $data_header=array(
    		...
    		$data=array(
    				'data_agen'=>$this->m_app->getJoinData2Table(
    										'*',
    										'tbl_magen','tbl_mpersonal_agen',
    										'tbl_magen.kode_personal_agen = tbl_mpersonal_agen.kode_personal_agen',
    										'tbl_musaha_agen',
    										'tbl_magen.kode_usaha_agen = tbl_musaha_agen.kode_usaha_agen')->result(),
    		);
        $this->load->view('v_header',$data_header);
        $this->load->view('v_top_menu');
        $this->load->view('v_left_menu');
        $this->load->view('agen/v_agen',$data);
        $this->load->view('v_footer');
        $this->load->view('v_footer_file');
	}

  public function save_add_agen()
  {
    ...
    $this->m_app->insertData($table_personal_agen,$data_personal_agen);

    $kodeusahaagen=$this->m_app->buatKodeUsahaAgen();
    $data_usaha_agen['kode_usaha_agen'] = $kodeusahaagen;
    ...
    $data_usaha_agen['createdate'] = date('Y-m-d H:i:s');
    $table_usaha_agen="tbl_musaha_agen";
    $this->m_app->insertData($table_usaha_agen,$data_usaha_agen);

    $koderekeningagen=$this->m_app->buatKodeRekeningAgen();
    $data_rekening_agen['kode_rekening_agen'] = $koderekeningagen;
    ...
    $data_rekening_agen['createdate'] = date('Y-m-d H:i:s');
    $table_rekening_agen="tbl_mrekening_agen";
    $this->m_app->insertData($table_rekening_agen,$data_rekening_agen);

    $kodeagen=$this->m_app->buatKodeAgen();
    $data_agen['kode_agen'] = $kodeagen;
    $data_agen['kode_personal_agen'] = $kodepersonalagen;
    $data_agen['kode_usaha_agen'] = $kodeusahaagen;
    $data_agen['kode_rekening_agen'] = $koderekeningagen;
    ...
    $data_agen['status_agen'] ='1';
    $data_agen['saldo_agen'] = '0';
    $data_agen['isactive'] = '1';
    $data_agen['createdate'] = date('Y-m-d H:i:s');
    $table_agen="tbl_magen";
    $proses=$this->m_app->insertData($table_agen,$data_agen);
  ...
  public function save_edit_agen()
  {
  ...
  public function transaksi_agen()
	{
      $data_header=array(
  		...
  		$data=array(
  				'data_transaksi_agen'=>$this->m_app->getLeftJoinData2Table(
  										'*',
                      'tbl_ttransaksi',
  										'tbl_magen',
  										'tbl_magen.kode_agen = tbl_ttransaksi.kode_agen',
                      'left',
  										'tbl_mpersonal_agen',
  										'tbl_magen.kode_personal_agen = tbl_mpersonal_agen.kode_personal_agen',
                      'left')->result(),
      ...
      $this->load->view('v_top_menu');
      $this->load->view('v_left_menu');
      $this->load->view('agen/v_transaksi_agen',$data);
      $this->load->view('v_footer');
      $this->load->view('v_footer_file');
	}
    ...
    public function komisi_agen()
  	{
        $data_header=array(
    		...
    		$data=array(
    				'data_komisi_agen'=>$this->m_app->getLeftJoinData2Table(
    										'*',
                        'tbl_komisi',
    										'tbl_ttransaksi',
    										'tbl_komisi.nomor_referensi = tbl_ttransaksi.nomor_referensi',
                        'left',
    										'tbl_magen',
    										'tbl_ttransaksi.kode_agen = tbl_magen.kode_agen',
                        'left')->result(),
    		);
        $this->load->view('v_header',$data_header);
        $this->load->view('v_top_menu');
        $this->load->view('v_left_menu');
        $this->load->view('agen/v_komisi_agen',$data);
        $this->load->view('v_footer');
        $this->load->view('v_footer_file');
  	}
